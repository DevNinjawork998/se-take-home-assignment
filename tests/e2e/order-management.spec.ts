...
test.describe("McDonald's Order Management System - E2E Tests", () => {
  ...
  test("should display the main interface correctly", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /new normal order/i })
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /new vip order/i })
    ).toBeVisible();
  ...
  test("Requirement 1: New Normal Order should appear in PENDING area", async ({
    page,
  }) => {
    ...
    await expect(orderCard).toContainText("NORMAL-001");
    await expect(orderCard).toContainText("NORMAL");
    await expect(orderCard).toContainText("PENDING");
  });

  test("Requirement 2: New VIP Order should appear in PENDING area before Normal orders", async ({
    page,
  }) => {
    ...
    await expect(secondOrder).toContainText("NORMAL-001");
    await expect(secondOrder).toContainText("NORMAL");
  });

  test("Requirement 3: Order numbers should be unique and increasing", async ({
    page,
  }) => {
    ...
    expect(orderTexts[1]).toContain("NORMAL-001"); // First normal order
    expect(orderTexts[2]).toContain("NORMAL-003"); // Second normal order
  });

  test("Requirement 4 & 5: Bot processes orders and becomes IDLE when no orders", async ({
    page,
  }) => {
    // Create an order
    await page.getByRole("button", { name: /new normal order/i }).click();
    ...
    await expect(completeArea.locator('[data-testid^="order-"]')).toHaveCount(
      1
    );

    // Verify order is marked as complete
    const completedOrder = completeArea
  ...
  test("Requirement 6: Multiple VIP orders maintain correct queue order", async ({
    page,
  }) => {
    ...
    expect(orderTexts[2]).toContain("NORMAL-003"); // Normal order last
  });

  test("Requirement 7: Removing bot stops processing and returns order to pending", async ({
    page,
  }) => {
    // Create an order
    await page.getByRole("button", { name: /new normal order/i }).click();
  ...
  test("Complex scenario: Multiple bots and orders interaction", async ({
    page,
  }) => {
    ...
    // Wait for at least one order to start processing (check for blue styling)
    await expect(
      pendingArea.locator('[data-testid^="order-"].border-blue-400')
    ).toHaveCount(2, { timeout: 5000 });

    // Check that all orders are still visible in pending area
    const allOrders = pendingArea.locator('[data-testid^="order-"]');
    const count = await allOrders.count();
    expect(count).toBe(3); // All orders should still be visible in pending area

    // Check that at least one order shows PROCESSING status
    const processingOrders = pendingArea.locator(
      '[data-testid^="order-"]:has-text("PROCESSING")'
    );
    const processingCount = await processingOrders.count();
    expect(processingCount).toBe(2); // Two orders should be processing

    // Add another order while bots are busy
    await page.getByRole("button", { name: /new vip order/i }).click();

    // Wait for all orders to complete (20 seconds + buffer for sequential processing)
  ...
  test("Bot management: Adding and removing multiple bots", async ({
    page,
  }) => {
    // Add 3 bots
    await page.getByRole("button", { name: /\+ bot/i }).click();
  ...
  test("Real-time updates: UI updates without page refresh", async ({
    page,
  }) => {
    // Create an order
    await page.getByRole("button", { name: /new normal order/i }).click();
