...
describe('OrderQueue', () => {
  ...
  const createMockOrder = (id: string, type: OrderType, status: OrderStatus = OrderStatus.PENDING): Order => ({
    id,
  ...
  test('should add normal orders to the end of queue', () => {
    const order1 = createMockOrder('NOR-001', OrderType.NORMAL);
  ...
  test('should add VIP orders before normal orders but maintain VIP order', () => {
    ...
    expect(pendingOrders[2].id).toBe('NOR-001'); // Normal order last
  });
  ...
  test('should remove orders correctly', () => {
    ...
    const removedOrder = orderQueue.removeOrder('NOR-001');
    expect(removedOrder?.id).toBe('NOR-001');
  ...
  test('should update order status correctly', () => {
    const order = createMockOrder('NOR-001', OrderType.NORMAL);
    orderQueue.addOrder(order);
    
    const success = orderQueue.updateOrderStatus('NOR-001', OrderStatus.PROCESSING);
